...
export class GameService {
  ...
  public readonly quizReady$ = new Subject<{roomId: string}>();
  ...
  createRoom(
    ...
    this.rooms.set(roomId, {
      ...
      isQuizReady: questions.length > 0
    });
  ...
  public updateRoomQuiz(roomId: string, questions: Question[]) {
    const room = this.rooms.get(roomId);
    if (room) {
      room.questions = questions;
      room.isQuizReady = true;
      this.quizReady$.next({ roomId });
    }
  }
