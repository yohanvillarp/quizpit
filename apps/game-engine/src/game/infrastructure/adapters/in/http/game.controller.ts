import { Controller, Post, Body, InternalServerErrorException, Get, HttpException, HttpStatus, UseGuards, Logger, Param, Delete, Query, Put } from '@nestjs/common';
...
export class GameController {
  ...
  @Put(':roomId/quiz')
  async updateRoomQuiz(@Param('roomId') roomId: string, @Body() payload: { questions: Question[] }) {
    try {
      this.gameService.updateRoomQuiz(roomId, payload.questions);
      return { success: true };
    } catch (e) {
      this.logger.error('Error updating room quiz', e);
      throw new InternalServerErrorException('No se pudo actualizar el quiz');
    }
  }
}
