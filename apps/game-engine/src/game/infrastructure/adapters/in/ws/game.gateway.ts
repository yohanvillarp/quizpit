...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  constructor(
    ...
    this.gameService.quizReady$.subscribe(({ roomId }) => {
      this.server?.to(roomId).emit('quiz_ready', {});
      const room = this.gameService.getRoom(roomId);
      if (room) {
        this.server?.to(roomId).emit('room_state', { ...room, players: Array.from(room.players.values()) });
      }
    });
  }
